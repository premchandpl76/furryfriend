    ...
    <>
      <div className="container">
        <header className="section has-text-centered">
          <h1 className="title is-size-3 has-text-primary">Search for pictures of good doggos</h1>
              ...
              {!isLoading &&
                pictures.map(picture => (
                  <div className='column is-one-quarter' key={picture.id}>
                    <DogCardInfo imageUrl={picture.url} pictureId={picture.id} />
                  </div>
                ))}
            </div>
          </div>
